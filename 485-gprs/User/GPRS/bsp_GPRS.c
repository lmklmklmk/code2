...
#include "bsp_GPRS.h"
  ...
  * @retval 无
  */
static void GPRS_NVIC_Configuration(void)
{
...
/*****************  发送一个字节 **********************/
void GPRS_Usart_SendByte( USART_TypeDef * pUSARTx, uint8_t ch)
{
	/* 发送一个字节数据到USART */
...
/****************** 发送8位的数组 ************************/
void GPRS_Usart_SendArray( USART_TypeDef * pUSARTx, uint8_t *array, uint16_t num)
{
  uint8_t i;
  ...
  {
	    /* 发送一个字节数据到USART */
	    GPRS_Usart_SendByte(pUSARTx,array[i]);	
...
/*****************  发送字符串 **********************/
void GPRS_Usart_SendString( USART_TypeDef * pUSARTx, char *str)
{
	unsigned int k=0;
  ...
  do 
  {
      GPRS_Usart_SendByte( pUSARTx, *(str + k) );
      k++;
  } while(*(str + k)!='\0');
...
/*****************  发送一个16位数 **********************/
void GPRS_Usart_SendHalfWord( USART_TypeDef * pUSARTx, uint16_t ch)
{
...
//{
//		/* 发送一个字节数据到串口 */
//		USART_SendData(GPRS_USARTx, (uint8_t) ch);
//		
//		/* 等待发送完毕 */
...
//{
//		/* 等待串口输入数据 */
//		while (USART_GetFlagStatus(GPRS_USARTx, USART_FLAG_RXNE) == RESET);
//
//		return (int)USART_ReceiveData(GPRS_USARTx);
//}
